...
def convert_detectron2_names(weights):
    """
    Map Detectron2 weight names to names in our model.

    Args:
        weights (dict): name -> tensor

    Returns:
        dict: our names -> tensor
        dict: our names -> Detectron2 names
    """
    logger = logging.getLogger(__name__)
    ...
    layer_keys = [
        k.replace(
            ...
            "proposal_generator.rpn_head.object_conv"
        ) for k in layer_keys
    ]
    layer_keys = [
        k.replace(
            "proposal_generator.rpn_head.anchor_deltas",
            "proposal_generator.rpn_head.object_deltas"
        ) for k in layer_keys
    ]
    # --------------------------------------------------------------------------
    # Done with replacements
...
def align_and_update_state_dicts(
    model_state_dict, ckpt_state_dict, c2_conversion=True, d2_conversion=True
):
    """
    ...
    if c2_conversion:
        ...
        ckpt_state_dict, original_keys = convert_detectron2_names(ckpt_state_dict)
    else:
